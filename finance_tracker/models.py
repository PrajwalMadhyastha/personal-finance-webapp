...
class User(db.Model, UserMixin):
    ...
    tags = db.relationship(
        "Tag", backref="user", lazy=True, cascade="all, delete-orphan"
    )
    recurring_transactions = db.relationship(
        "RecurringTransaction", backref="user", lazy=True, cascade="all, delete-orphan"
    )
    investment_transactions = db.relationship(
        "InvestmentTransaction", backref="user", lazy=True, cascade="all, delete-orphan"
    )
...
class Category(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    budgets = db.relationship(
        "Budget", backref="category", lazy=True, cascade="all, delete-orphan"
    )
...
class RecurringTransaction(db.Model):
    ...
    generated_transactions = db.relationship(
        "Transaction", backref="recurring_rule", lazy="dynamic"
    )
...
class InvestmentTransaction(db.Model):
    ...
    quantity = db.Column(
        db.Numeric(18, 8), nullable=False
