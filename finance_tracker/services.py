...
def get_stock_price(ticker_symbol):
    ...
    try:
        ...
        response = requests.get(ALPHA_VANTAGE_BASE_URL, params=params, timeout=10) # 10-second timeout
        ...
        if "Note" in data:
            logger.warning(f"Alpha Vantage API note for {ticker_symbol}: {data['Note']}")
